...
describe('buildAuthUrl', () => {
  ...
  test('encodes special characters in redirect_uri', () => {
    const url = buildAuthUrl({ ...baseParams, redirectUri: 'http://localhost:5173/auth/callback?foo=bar' });
    // searchParams getter decodes, so just check it round-trips
    const parsed = new URL(url);
    expect(parsed.searchParams.get('redirect_uri')).toBe('http://localhost:5173/auth/callback?foo=bar');
  });

  test('works with a production Gitea URL', () => {
    const url = new URL(buildAuthUrl({ ...baseParams, giteaUrl: 'https://git.example.com' }));
