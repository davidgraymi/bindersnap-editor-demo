import { Extension, isMarkActive, Mark, mergeAttributes, getMarkRange, getMarksBetween } from '@tiptap/core';
...
declare module '@tiptap/core' {
  interface Commands<ReturnType> {
    versionHistory: {
       ...
       * @returns 
       */
      setDiffContent: (ours: string, theirs: string, base?: string) => ReturnType;
      /**
...
export const FormatChange = Mark.create({
  ...
  renderHTML({ HTMLAttributes }) {
    return ['format-change', mergeAttributes(this.options.HTMLAttributes, HTMLAttributes), 0];
  },
});
 ...
 * @returns boolean
 */
const applyMergeOperation = (opType: MERGE_COMMAND_TYPE, param: CommandProps): boolean => {
  /**
   ...
   */
  if ((opType === MERGE_COMMAND_THEIRS || opType === MERGE_COMMAND_OURS) && from === to) {
    // detect left mark
  ...
  } else if (opType === MERGE_COMMAND_THEIRS_ALL || opType === MERGE_COMMAND_OURS_ALL) {
    // all editor content
    markRanges = getMarksBetween(0, param.editor.state.doc.content.size, param.editor.state.doc)
    // change the opType to normal
    opType = opType === MERGE_COMMAND_THEIRS_ALL ? MERGE_COMMAND_THEIRS : MERGE_COMMAND_OURS 
  } else {
    // just the selection
  ...
  markRanges.forEach((markRange) => {
    const isAcceptInsert = opType === MERGE_COMMAND_THEIRS && markRange.mark.type.name === MARK_INSERTION
    const isRejectDelete = opType === MERGE_COMMAND_OURS && markRange.mark.type.name === MARK_DELETION
...
export const VersionHistory = Extension.create<{ view: VERSION_CONTROL_TYPE, onViewChange: (view: VERSION_CONTROL_TYPE) => void }>({
  name: EXTENSION_NAME,
  ...
  addCommands() {
    return {
      setDiffContent: (ours: string, theirs: string, base?: string) => ({ commands }) => {
        // TODO: handle base
