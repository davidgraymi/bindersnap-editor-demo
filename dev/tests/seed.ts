...
async function giteaRequest(baseUrl: string, path: string, options: RequestOptions = {}): Promise<Response> {
  const {
    method = 'GET',
    auth,
    headers,
  ...
  if (!expectedStatuses.includes(response.status)) {
    ...
    throw new Error(`Request failed ${method} ${path}: ${response.status} ${responseBody}`);
  }

  return response;
}

async function giteaJson<T>(baseUrl: string, path: string, options: RequestOptions = {}): Promise<T> {
...
async function waitForUrl(baseUrl: string, path: string, attempts: number, delayMs: number): Promise<void> {
...
async function maybeBootstrapInstall(
  ...
  const adminLookup = await giteaRequest(baseUrl, `/api/v1/users/${encodeURIComponent(adminUser)}`, {
...
async function ensureUser(
  ...
  log(response.status === 201 ? `Created user: ${username}` : `User already exists: ${username}`);
}
...
async function ensureRepo(
  ...
  log(response.status === 201 ? `Created repo: ${repoName}` : `Repo already exists: ${repoName}`);
}
...
async function ensureBranch(
  ...
  log(createResponse.status === 201 ? `Created branch: ${branchName}` : `Branch already exists: ${branchName}`);
}
...
async function createAccessToken(
  ...
  if (!token.sha1) {
    throw new Error('Token creation succeeded but no token value was returned.');
  }
...
export async function seedDevStack(options: SeedOptions = {}): Promise<SeedResult> {
  ...
  const adminPass = options.adminPass ?? process.env.GITEA_ADMIN_PASS ?? DEFAULT_ADMIN_PASS;
  const bobUser = options.bobUser ?? process.env.GITEA_BOB_USER ?? DEFAULT_BOB_USER;
  const bobPass = options.bobPass ?? process.env.GITEA_BOB_PASS ?? DEFAULT_BOB_PASS;
  ...
  await ensureUser(baseUrl, adminAuth, bobUser, bobPass, 'bob@example.com', 'Bob Reviewer', log);
  ...
  await ensureCollaborator(baseUrl, adminAuth, adminUser, repoName, bobUser, log);
  await ensureBranch(baseUrl, adminAuth, adminUser, repoName, FEATURE_BRANCH, 'main', log);
  await ensureFile(
    baseUrl,
    adminAuth,
    adminUser,
