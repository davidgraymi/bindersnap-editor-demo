...
const ciTerraformVars = readFileSync("infra/ci/terraform.tfvars.example", "utf8");
const configBucketTerraform = readFileSync("infra/config-bucket/main.tf", "utf8");
...
describe("config-as-code deploy wiring", () => {
  test("workflow publishes the tracked bundle to S3 and applies it over SSM", () => {
    ...
    expect(configDeployWorkflow).toContain('aws s3 sync ./ "s3://${CONFIG_BUCKET}/"');
    expect(configDeployWorkflow).toContain('--include "docker-compose.prod.yml"');
  ...
  test("deploy role can publish objects into the config bucket", () => {
    ...
    expect(ciTerraformVars).toContain('# config_bucket_name = "bindersnap-config"');
    ...
    expect(ciTerraform).toContain('arn:${data.aws_partition.current.partition}:s3:::${var.config_bucket_name}');
    expect(ciTerraform).toContain('arn:${data.aws_partition.current.partition}:s3:::${var.config_bucket_name}/*');
  });
  ...
  test("the config bundle and first-boot seed include the host CLI, and user-data drift is no longer ignored", () => {
    ...
    expect(userData).toContain('install -m 0755 "${APP_DIR}/scripts/bindersnap" /usr/local/bin/bindersnap');
