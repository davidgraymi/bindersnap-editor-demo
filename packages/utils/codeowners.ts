 ...
 *
 * Three of these are the answers a customer gives to "what has to be signed
 * off?" — everything here, this drawer, this policy. Anything narrower is a
 * folder; anything broader is the binder.
 *
 * **`rules` is the fourth, and it is about this file itself.** Who has to sign
 * something off is a control, and a control nobody has to approve a change to
 * is not one: without this scope, anybody who can open a change request can
 * propose rewriting the sign-off rules, and the only thing standing in the way
 * is the binder's ordinary approval count. A binder that requires the
 * Infection Control group to approve its nursing policies, and one approval
 * from anybody to approve *removing that requirement*, has a gate with a door
 * beside it.
 *
 * It is a separate scope rather than a folder rule over `.gitea` because the
 * file is the only thing in there that matters and naming it exactly is what
 * makes the rule legible in a change request.
 */
export type SignOffScope = "binder" | "folder" | "document" | "rules";
...
export interface SignOffRule {
   ...
   * - `rules` — the empty string. There is only one file of rules.
   *
 ...
 * | rules    | `\.gitea/CODEOWNERS`       | this file, and nothing else          |
 *
 * **The rules pattern names one file exactly**, anchored at both ends by the
 * `^…$` Gitea wraps every pattern in. `.gitea/CODEOWNERS.bak` does not match
 * it, and neither does anything else somebody might one day put in `.gitea`.
 *
...
export function rulePattern(rule: {
  ...
  switch (rule.scope) {
    ...
    case "document":
      return `.*\\.${rule.target}(\\..*)?`;
    case "rules":
      // The file this module writes, matched literally. `escapeRegex` is what
      // turns the leading dot into `\.` — without it the pattern would also
      // match `xgitea/CODEOWNERS`, which is not a thing that exists but is
      // exactly the class of near miss this module exists to avoid.
      return escapeRegex(CODEOWNERS_PATH);
  }
}
...
function describeTarget(rule: { scope: SignOffScope; target: string }): string {
  switch (rule.scope) {
    ...
    case "document":
      ...
      return "a document";
    case "rules":
      return "the sign-off rules themselves";
  }
}
...
function checkPatternAim(
  ...
  if (rule.scope === "rules") {
    // It has to hit the file, and nothing that merely looks like it. A rule
    // that also caught every policy would demand the rules group's sign-off on
    // ordinary work; one that caught nothing would leave the gate ungated
    // while every screen said it was not.
    if (!compiled.test(CODEOWNERS_PATH)) {
      problems.push(
        `${where} produced a rule that does not match the sign-off rules file. Changing who signs things off would need nobody's approval.`,
      );
    }
    ...
    for (const near of [
      ...
      if (compiled.test(near)) {
        problems.push(
          `${where} produced a rule that also matches \u201c${near}\u201d. It would demand sign-off on things nobody chose.`,
        );
        break;
      }
    }
...
export function renderCodeowners(
  ...
  const lines = [
    ...
    "#",
    "# Each line is something in this binder — the whole binder, one folder,",
    "# one document, or this file of rules itself — and the groups or people",
    "# who sign off on changes to it.",
...
export function ruleFromPattern(
  ...
  if (pattern === rulePattern({ scope: "rules", target: "" })) {
    return { scope: "rules", target: "" };
  }
...
export function validateSignOffRules(params: {
  ...
  for (const rule of rules) {
    ...
    // Two scopes name one thing and take no target: there is one binder, and
    // there is one file of rules.
    if (
      (rule.scope === "binder" || rule.scope === "rules") &&
      trimFolder(rule.target) !== ""
    ) {
      problems.push(
        rule.scope === "binder"
          ? `A rule over the whole binder cannot also name “${rule.target}”. Set it on that folder or that document instead.`
          : `A rule over the sign-off rules cannot also name “${rule.target}”. There is one set of rules.`,
      );
      continue;
    }
